...
def callback(data):
	global heard_yaw, heard_yawrate, heard_velocity, heard_stamp
	heard_stamp    = data.data[0]
	heard_velocity = data.data[1]
	heard_yaw 	   = data.data[2]
	heard_yawrate  = data.data[3]
	main()


def listener():
	rospy.init_node('llc_listener')
    #rospy.Subscriber('arl_stepper_control', Float32MultiArray, callback2)
    #rospy.Subscriber('arl_velocity_control', Float32MultiArray, callback1)
    rospy.Subscriber('llc_control', Float32MultiArray, callback)
    rospy.spin()
...
def init(init_request_msg):
	global pwm_out, velocity_required, drive_address, steer_address, auto_mode, stepper, steer_dependencies, to_control_steer, feedback, yaw_err_index, yawrate_err_index, current_index, previous_index, req_index, previous_req_index, current_angle_index, current_rate_index, previous_angle_index, previous_rate_index, steer_parameters, steer_system, ts, velocity, pwm, heard_yaw, heard_yawrate, heard_velocity, heard_stamp, velocity_input, state_index, direction_index, steps_index, delay_index, controlling, rule1, rule2, rule3, rule4, velocity_ctrl, talker_stamp
	if init_request_msg == 'init_controller_fuzzy':
	...
	elif init_request_msg == 'init_data':
		pwm_out 			 = 0.0
...
def update_values():
	global pwm_out, velocity_required, drive_address, steer_address, auto_mode, stepper, steer_dependencies, to_control_steer, feedback, yaw_err_index, yawrate_err_index, current_index, previous_index, req_index, previous_req_index, current_angle_index, current_rate_index, previous_angle_index, previous_rate_index, steer_parameters, steer_system, ts, velocity, pwm, heard_yaw, heard_yawrate, heard_velocity, heard_stamp, velocity_input, state_index, direction_index, steps_index, delay_index, controlling, rule1, rule2, rule3, rule4, velocity_ctrl, talker_stamp
	talker_stamp[current_index] 						 = heard_stamp
	stepper[0] 											 = heard_yaw
	stepper[1]											 = heard_yawrate
	velocity_required									 = heard_velocity
...
def previous_states_assignment():
	global pwm_out, velocity_required, drive_address, steer_address, auto_mode, stepper, steer_dependencies, to_control_steer, feedback, yaw_err_index, yawrate_err_index, current_index, previous_index, req_index, previous_req_index, current_angle_index, current_rate_index, previous_angle_index, previous_rate_index, steer_parameters, steer_system, ts, velocity, pwm, heard_yaw, heard_yawrate, heard_velocity, heard_stamp, velocity_input, state_index, direction_index, steps_index, delay_index, controlling, rule1, rule2, rule3, rule4, velocity_ctrl, talker_stamp
    ...
    talker_stamp[previous_index]      	           			   = talker_stamp[current_index]
...
#       	write_to(steer_system, steer_parameters)
        previous_states_assignment()


if __name__ =='__main__':
	init('init_data')
	init('init_controller_fuzzy')
	while True:
	    listener()
